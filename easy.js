...
const altNumbers = (numArray) => {
    ...
    if (positiveNums === negativeNums || positiveNums === negativeNums + 1) {
        for (let i = 0; i < positiveNums; i++) {
            retArray.push(positiveArray[i]);
            if (i < negativeNums) {
                retArray.push(negativeArray[i]);
            }
        }
    }
    else if (negativeNums === positiveNums + 1) {
        for (let i = 0; i < negativeNums; i++) {
            retArray.push(negativeArray[i]);
            if (i < positiveNums) {
                retArray.push(positiveArray[i]);
            }
